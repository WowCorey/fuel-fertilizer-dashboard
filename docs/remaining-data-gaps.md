...
| Daily stock-out updates | partial/manual | The current PM&C stock-out table is a dated public snapshot with seven-day changes. The WA weekly fuel update adds a WA-only dated statewide count and station denominator. Neither source is a daily national API, and the PM&C table does not publish an Australia-wide petrol total. | Hand-key dated snapshots when reviewed. Add ingestion only when a stable state/territory or PM&C endpoint with clear reuse terms is found. |
...
| Live national station outage feed | unavailable, with WA partial snapshot | The current public material exposes dated stock-out counts by state/territory. The WA weekly update now provides one state-level dated snapshot. These sources do not expose a live national station outage API with station-level status. | Keep `fuel_security_live_station_outage_feed` unavailable. Add more partial state modules only when a public source exposes outage status with stable terms. |
