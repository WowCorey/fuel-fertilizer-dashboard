...
| Project/company production by state | partial | Current AES and AECR envelopes support state and basin context, not company/project-level flows or tax paid by state. | Add project/company rows only from a public source with exact product, location, period, unit and reuse terms. |
