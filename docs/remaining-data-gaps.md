...
| Gap | Current status | Why not filled yet | Next action |
|---|---|---|---|
| State and territory net debt | partial (4/8 verified) | Pass 2B value-entry pass populated 4 of 8 jurisdiction rows from official Budget Paper PDFs downloaded directly: NSW A$120.3b GGS (Budget Paper No. 1 Chapter 8 Table 8.1), VIC A$167.6b GGS (2025-26 Strategy and Outlook), QLD A$41.8b GGS (Budget Paper No. 2 Balance Sheet), WA A$39.0b Total Public Sector (Budget Paper No. 3 Table 2 - Partial coverage because WA's headline is TPS, not GGS, and BP3 does not publish a clean GGS-only single-year figure in its overview tables). SA, TAS, ACT, NT remain Partial coverage with null values; their Treasury budget homepages returned HTTP 403/503 to scripted access from the build sandbox. No national aggregate is published. | Hand-key SA/TAS/ACT/NT general government sector net debt figures from each Budget Paper PDF opened in a browser into `data/manual/state_government_debt_summary.json` `extra.fields.rows[*].value`. Promote `coverage_label` to Observed only when GGS net debt is loaded with a page/table reference. Continue to withhold any national aggregate. |
