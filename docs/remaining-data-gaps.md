...
| Daily stock-out updates | deferred | The current PM&C stock-out table is a dated public snapshot with seven-day changes. It is not a daily API, and the current table does not publish an Australia-wide petrol total. | Hand-key dated snapshots when reviewed, or add ingestion only when a stable state/territory or PM&C endpoint is found. |
