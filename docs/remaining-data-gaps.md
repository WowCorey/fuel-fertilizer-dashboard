...
| Gap | Current status | Why not filled yet | Next action |
|---|---|---|---|
| State and territory net debt | partial (scaffold loaded) | Pass 2 has scaffolded the envelope with all 8 jurisdiction rows (NSW, VIC, QLD, WA, SA, TAS, ACT, NT) carrying named Treasury Budget homepage URLs. Per-jurisdiction values remain null because the figures live inside Budget Paper PDFs that this build environment cannot fetch directly. NSW/VIC/QLD/WA Treasury homepages were reachable (HTTP 200); SA/TAS/ACT/NT Treasury homepages returned HTTP 403/503 to scripted access from the build sandbox. The state-debt detail table renders on the page with the eight rows, named source URLs and explicit "value pending PDF verification" caveats. No national aggregate is published; rows are not safe to sum across jurisdictions. | Hand-key each jurisdiction's general government sector net debt figure from its named Budget Paper PDF (browser-opened) into `data/manual/state_government_debt_summary.json` `extra.fields.rows[*].value`. Update `coverage_label` from "Partial coverage" to "Observed" only when a verified figure is loaded with a page/table reference. Do not publish a national aggregate. |
