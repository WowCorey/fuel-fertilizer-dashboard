...
| [AU economics](ui_kits/au-economics-dashboard/index.html) | v1.1 | RBA cash rate, household and government debt, mortgages, unemployment and inflation, and a state/territory debt scaffold | RBA cash rate (F1.1), household debt-to-income (E2), standard variable mortgage rate (F5), credit-card balances accruing interest (C1.1), ABS real GDP growth, unemployment and CPI load from verified RBA/ABS endpoints; AOFM AGS face value is hand-keyed from official stock_ags.csv after workbook fetch timeouts; state-level net debt is now scaffolded with named Treasury budget-paper URLs for all 8 jurisdictions (NSW, VIC, QLD, WA, SA, TAS, ACT, NT) and a comparability table on the page, but per-jurisdiction values remain pending hand-key from each Budget Paper PDF, and no national aggregate is published because the rows are not safe to sum across jurisdictions |
