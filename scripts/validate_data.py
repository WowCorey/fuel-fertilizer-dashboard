...
def validate_phase2_extractors(
    ...
    if not isinstance(fields, dict):
        add(errors, rel(path), "manual_automation.v2 envelope extra.fields must be an object")
        return
    extractor = fields.get("extractor")
    if not isinstance(extractor, str) or not extractor.strip():
        add(errors, rel(path), "manual_automation.v2 extra.fields.extractor is required")
    resolved_url = fields.get("resolved_url")
    if not isinstance(resolved_url, str) or not resolved_url.startswith("http"):
        add(errors, rel(path), "manual_automation.v2 extra.fields.resolved_url must be an http URL")
